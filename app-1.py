...
with tab1:
    ...
    # Waterfall — структура чистой прибыли
    st.markdown("""<div class="section-title">Waterfall — структура чистой прибыли</div>""", unsafe_allow_html=True)
    ...
    top_brands_wf = (df_sales[
        df_sales["Город"].isin(sel_cities) &
        df_sales["Куратор"].isin(sel_curators) &
        df_sales["Бренд_ч"].isin(sel_brands) &
        (df_sales["Проводка"] == "Расход")
    ].groupby("Бренд_ч")["Дельта"].sum().sort_values(ascending=False))

    top8_wf   = top_brands_wf.head(8)
    others_wf = top_brands_wf.iloc[8:].sum()
    labels_wf = list(top8_wf.index) + ["Прочие", "Возвраты", "Чистая прибыль"]
    values_wf = list(top8_wf.values) + [others_wf, ret_l, net_p]

    bottoms_wf, running_wf = [], 0
    for lbl, val in zip(labels_wf, values_wf):
        bottoms_wf.append(0 if lbl == "Чистая прибыль" else running_wf)
        if lbl != "Чистая прибыль": running_wf += val

    bar_colors_wf = ["#2563EB" if l=="Чистая прибыль" else ("#22C55E" if v>=0 else "#EF4444")
                     for l, v in zip(labels_wf, values_wf)]

    fig_wf, ax_wf = plt.subplots(figsize=(14, 6), facecolor=BG)
    ax_wf.set_facecolor(BG)
    x_wf = np.arange(len(labels_wf))
    ax_wf.bar(x_wf, values_wf, bottom=bottoms_wf, color=bar_colors_wf,
              width=0.6, zorder=3, edgecolor="white", linewidth=0.8)

    run2 = 0
    for i, (lbl, val) in enumerate(zip(labels_wf[:-1], values_wf[:-1])):
        ax_wf.plot([i+0.3, i+0.7], [run2+val, run2+val],
                   color="#CBD5E1", linewidth=1.2, linestyle="--", zorder=2)
        run2 += val

    run3 = 0
    for i, (lbl, val) in enumerate(zip(labels_wf, values_wf)):
        if lbl == "Чистая прибыль":
            ax_wf.text(i, val+1.2e6, f"{val/1e6:.1f} млн", ha="center",
                       va="bottom", fontsize=8.5, fontweight="bold", color="#2563EB")
        elif val >= 0:
            ax_wf.text(i, run3+val+0.5e6, f"+{val/1e6:.1f}",
                       ha="center", va="bottom", fontsize=7.5, fontweight="bold", color="#166534")
            run3 += val
        else:
            ax_wf.text(i, run3+val-0.5e6, f"{val/1e6:.1f}",
    ...
    ax_wf.axhline(0, color=DARK, linewidth=1, zorder=4)
    ax_wf.set_xticks(x_wf)
    ax_wf.set_xticklabels(labels_wf, rotation=25, ha="right", fontsize=9.5)
    ax_wf.yaxis.set_major_formatter(plt.FuncFormatter(lambda v,_: f"{v/1e6:.0f} млн"))
    ax_wf.set_ylabel("KZT", fontsize=10)
    ax_wf.grid(axis="y", alpha=0.2, zorder=0)
    ax_wf.legend(handles=[
        mpatches.Patch(color="#22C55E", label="Прибыль от продаж"),
        mpatches.Patch(color="#EF4444", label="Потери от возвратов"),
        mpatches.Patch(color="#2563EB", label="Чистая прибыль"),
    ], loc="upper right", fontsize=9, framealpha=0.9)
    ax_wf.set_title(f"Возвраты = {leak_pct:.1f}% валовой прибыли  |  Чистая прибыль: {net_p/1e6:.1f} млн ₸",
                    fontsize=11, fontweight="bold", pad=10, color=DARK)
    plt.tight_layout()
    st.pyplot(fig_wf); plt.close()
